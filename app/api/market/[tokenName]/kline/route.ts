...
export async function GET(
  request: NextRequest,
  context: { params: Promise<{ tokenName: string }> }
): Promise<NextResponse<unknown>> {
  try {
    const { tokenName } = await context.params;

    const clientQuery = request.nextUrl.searchParams;
    const _backendURL = new URL(
      `/api/v1/market/token-100kline`,
      backend
    );
    _backendURL.searchParams.append("tokenName", tokenName);
